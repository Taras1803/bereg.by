...
class ControllerInformationContact extends Controller
{
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            elseif ($this->request->post['action'] == 'send_sms') {
                ...
                if ($this->request->post['type'] == 'req') {
                    ...
                    $answer = $this->sendRocketSMS($this->request->post['phone'], $sms);

                    if (isset($answer['error'])){
                        $json['error'] = 1;
                        $json['answer'] = json_encode($answer);
                    }elseif(isset($answer['id'])){
                        $json['error'] = 0;
                    }else{
                        $json['error'] = 0;
                    }
   ...
   public function sendRocketSMS($phone, $message) {
        $curl = curl_init();

        curl_setopt($curl, CURLOPT_URL, 'http://api.rocketsms.by/json/send');
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($curl, CURLOPT_POST, true);
        curl_setopt($curl, CURLOPT_POSTFIELDS, "username=". $this->config->get('config_sms_gate_username') ."&password=". $this->config->get('config_sms_gate_password') ."&phone=" . $phone . "&text=" . $message);

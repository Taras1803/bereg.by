...
class ModelCheckoutOrder extends Model {
    ...
    public function getStatusName($order_status_id) {
        ...
        {
            $query = $this->db->query(" SELECT `name` FROM `" . DB_PREFIX . "order_status` WHERE order_status_id = 1 AND language_id = '" . (int)$this->config->get('config_language_id') . "' LIMIT 1");
            $ss = $query->row['name'];
            return $ss;
        }
        if ($order_status_id == 2)
        {
            $query = $this->db->query(" SELECT `name` FROM `" . DB_PREFIX . "order_status` WHERE order_status_id = 2 AND language_id = '" . (int)$this->config->get('config_language_id') . "' LIMIT 1");
            $ss = $query->row['name'];
            return $ss;
        }
        if ($order_status_id == 3)
        {
            $query = $this->db->query(" SELECT `name` FROM `" . DB_PREFIX . "order_status` WHERE order_status_id = 3 AND language_id = '" . (int)$this->config->get('config_language_id') . "' LIMIT 1");
            $ss = $query->row['name'];
            return $ss;
        }
        if ($order_status_id == 5)
        {
            $query = $this->db->query(" SELECT `name` FROM `" . DB_PREFIX . "order_status` WHERE order_status_id = 5 AND language_id = '" . (int)$this->config->get('config_language_id') . "' LIMIT 1");
            $ss = $query->row['name'];
            return $ss;
        }
        if ($order_status_id == 7)
        {
            $query = $this->db->query(" SELECT `name` FROM `" . DB_PREFIX . "order_status` WHERE order_status_id = 7 AND language_id = '" . (int)$this->config->get('config_language_id') . "' LIMIT 1");
            $ss = $query->row['name'];
            return $ss;
        }
        if ($order_status_id == 8)
        {
            $query = $this->db->query(" SELECT `name` FROM `" . DB_PREFIX . "order_status` WHERE order_status_id = 8 AND language_id = '" . (int)$this->config->get('config_language_id') . "' LIMIT 1");
            $ss = $query->row['name'];
            return $ss;
        }
        if ($order_status_id == 9)
        {
            $query = $this->db->query(" SELECT `name` FROM `" . DB_PREFIX . "order_status` WHERE order_status_id = 9 AND language_id = '" . (int)$this->config->get('config_language_id') . "' LIMIT 1");
            $ss = $query->row['name'];
            return $ss;
        }
        if ($order_status_id == 10)
        {
            $query = $this->db->query(" SELECT `name` FROM `" . DB_PREFIX . "order_status` WHERE order_status_id = 10 AND language_id = '" . (int)$this->config->get('config_language_id') . "' LIMIT 1");
            $ss = $query->row['name'];
            return $ss;
        }
        if ($order_status_id == 11)
        {
            $query = $this->db->query(" SELECT `name` FROM `" . DB_PREFIX . "order_status` WHERE order_status_id = 11 AND language_id = '" . (int)$this->config->get('config_language_id') . "' LIMIT 1");
            $ss = $query->row['name'];
            return $ss;
        }
        if ($order_status_id == 12)
        {
            $query = $this->db->query(" SELECT `name` FROM `" . DB_PREFIX . "order_status` WHERE order_status_id = 12 AND language_id = '" . (int)$this->config->get('config_language_id') . "' LIMIT 1");
            $ss = $query->row['name'];
            return $ss;
        }
        if ($order_status_id == 13)
        {
            $query = $this->db->query(" SELECT `name` FROM `" . DB_PREFIX . "order_status` WHERE order_status_id = 13 AND language_id = '" . (int)$this->config->get('config_language_id') . "' LIMIT 1");
            $ss = $query->row['name'];
            return $ss;
        }
        if ($order_status_id == 14)
        {
            $query = $this->db->query(" SELECT `name` FROM `" . DB_PREFIX . "order_status` WHERE order_status_id = 14 AND language_id = '" . (int)$this->config->get('config_language_id') . "' LIMIT 1");
            $ss = $query->row['name'];
            return $ss;
        }
        if ($order_status_id == 15)
        {
            $query = $this->db->query(" SELECT `name` FROM `" . DB_PREFIX . "order_status` WHERE order_status_id = 15 AND language_id = '" . (int)$this->config->get('config_language_id') . "' LIMIT 1");
            $ss = $query->row['name'];
            return $ss;
        }
        if ($order_status_id == 16)
        {
            $query = $this->db->query(" SELECT `name` FROM `" . DB_PREFIX . "order_status` WHERE order_status_id = 16 AND language_id = '" . (int)$this->config->get('config_language_id') . "' LIMIT 1");
            $ss = $query->row['name'];
            return $ss;
        }
    }
	...
	*/
			...
			if ($order_info['order_status_id'] && $order_status_id && $notify) {
                ...
                if ($this->getStatusName($order_status_id)!=NULL & $order_status_id!=1)
                {
                    $fullitems_query = $this->db->query("SELECT GROUP_CONCAT(' ', `name`, ' (', `quantity`, ')') as `fullitems`,
                                                            GROUP_CONCAT(' ', `model`, ' (', `quantity`, ')') as `liteitems`
                                                     FROM `" . DB_PREFIX . "order_product` where `order_id` = " . (int)$order_id);
                    ...
                    $ext = array(
                        'order_info' => $order_info,
                        'config_sms_alert_client' => $this->config->get('config_sms_alert_status'),
                        'config_sms_alert_admin' => $this->config->get('config_sms_alert_admin')
                    );
                    $options = array(
                        'to'       => $this->config->get('config_sms_to'),
                        ...
                        'message'  => str_replace(array('{ID}', '{DATE}', '{TIME}', '{SUM}', '{PHONE}', '{STATUS}'),
                            array($order_id, date('d.m.Y'), date('H:i'), floatval($order_info['total']), $order_info['telephone'],$this->getStatusName($order_status_id)),
                            $this->config->get('config_sms_message2status')),
                        'message2client'  => str_replace(array('{ID}', '{DATE}', '{TIME}', '{SUM}', '{PHONE}', '{STATUS}'),
                            array($order_id, date('d.m.Y'), date('H:i'), floatval($order_info['total']), $order_info['telephone'],$this->getStatusName($order_status_id)),
                            $this->config->get('config_sms_message2status')),
                        'ext'      => $ext
                    );
                    ...
                    if($this->request->post['notify_sms_comment']) $options['message2client'] = str_replace('{COMMENT}', $this->request->post['comment'], $options['message2client']);
                    else $options['message2client'] = str_replace('{COMMENT}', '', $options['message2client']);

$(document).ready(function() {
	...
	//dropdown menu
	$('header nav > li:first-child').on('mouseenter', function(e) {
		var $this = $(this);
		clearTimeout(timerId);

		timerId = setTimeout(showDropdown, 200);

		function showDropdown() {

			$('.dropdown-mnu').slideDown();

		}

	});

	$('header nav > li:first-child').on('mouseleave', function(e){
		var $this = $(this);
		clearTimeout(timerId);

		timerId = setTimeout(hideDropdown, 300);

		function hideDropdown() {

			$('.dropdown-mnu').slideUp();
	...
	$('.mobile-menu__open').on('click', function(e) {
		...
		$('.mobile-menu').removeClass('menu-slide_hide').addClass('menu-slide_show');
		popupOverlay.css('display', 'block');

	});
	$('.mobile-menu__close').on('click', function(e) {

		$('.mobile-menu').removeClass('menu-slide_show').addClass('menu-slide_hide');
	...
	//mobile tel
	$('.mobile-header .icon-tel_drop').on('click', function(e) {
		e.preventDefault();
		
		$('.mobile-tel').removeClass('tel-slide_hide').addClass('tel-slide_show');
		popupOverlay.css('display', 'block');

	});
	$('.mobile-tel .close').on('click', function(e) {

		$('.mobile-tel').removeClass('tel-slide_show').addClass('tel-slide_hide');
		popupOverlay.css('display', 'none');
		...
		if($('.send_sms').val() !== ''){
			...
			request.done(function (response) {
				if(response.error === 0 ){
					...
					$('#send_sms_btn').after('<p id="ajax_error" style="color: rgba(159,0,0,0.73)">отправка не удалась' + response.answer + '</p>')
					setTimeout(function () {
